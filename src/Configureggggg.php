...
class Configure
{
    ...
    public function setFiles(...$files):self
    {
        ...
        foreach ($files as $file) {
            if (is_array($file)) {
                list($cackeFile,$key) = $file;
                $cackeFiles[] = $cackeFile;
            } else {
                $cackeFiles[] = $file;
            }
        }

        $this->addCacheFiles(...$cackeFiles);

        return $this;
    }

    public function addCacheFiles(...$cacheFiles):self
    {
        $this->cacheFiles = array_merge($this->cacheFiles,$cacheFiles);
